...
const pasteListener = ()=>{
    document.addEventListener("paste", async (e)=>{
        ...
        for (const clipboardItem of clipboardItems)for (const type of clipboardItem.types){
...
const drawCheckPattern = (canvas, ctx)=>{
    ...
    for(let i = 0; i < 40; ++i)for(let j = 0, col = 20; j < col; ++j)ctx.rect(2 * j * size + (i % 2 ? 0 : size), i * size, size, size);
    ctx.fill();
};
const draw = ()=>{
    const canvas = document.querySelector("canvas");
    ...
    if (image) switch(shape){
        case "original":
            canvas.width = image.width;
            canvas.height = image.height;
            ctx.save();
            ctx.translate((canvas.width - image.width * z) / 2 * x / 100, (canvas.width - image.width * z) / 2 * y / 100);
            ctx.transform(z, 0, 0, z, -(z - 1) * canvas.width / 2, -(z - 1) * canvas.height / 2);
        ...
        default:
            {
                ...
                ctx.save();
                ctx.translate((canvas.width - image.width * z) / 2 * x / 100, (canvas.height - image.height * z) / 2 * y / 100);
                ctx.transform(z, 0, 0, z, -(z - 1) * canvas.width / 2, -(z - 1) * canvas.height / 2);
                drawCheckPattern(canvas, ctx);
                ctx.drawImage(image, 0, 0, image.width, image.height, canvasX, canvasY, image.width * ratio, image.height * ratio);
                ctx.restore();
                break;
            }
    }
    ...
    ctx.drawImage(banner, 0, 0, banner.width, banner.height, 0, bannerY, canvas.width, height);
    // Draw "Category Name"
...
const loadBanner = ()=>{
    ...
    settings.banner.src = require("617f2f9d4b444f52");
...
const loadMaterial = ()=>{
    ...
    settings.material.src = require("dc2459ca74fd4720");
...
},{"617f2f9d4b444f52":"g4Vz6","dc2459ca74fd4720":"dkgsz"}],"g4Vz6":[function(require,module,exports) {
module.exports = require("a9c34b82bc2ce74a").getBundleURL("hO9Nr") + "banner.ae405365.png" + "?" + Date.now();

},{"a9c34b82bc2ce74a":"lgJ39"}],"lgJ39":[function(require,module,exports) {
"use strict";
var bundleURL = {};
...
},{}],"dkgsz":[function(require,module,exports) {
module.exports = require("441cf2a00283e3e1").getBundleURL("hO9Nr") + "m3.b54ff260.svg" + "?" + Date.now();

},{"441cf2a00283e3e1":"lgJ39"}]},["iHur1","bAabt"], "bAabt", "parcelRequire5237")
